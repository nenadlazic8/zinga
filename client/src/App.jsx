      ...
      {confettiDone && (
        <div className="text-center space-y-6 animate-fade-in">
          ...
          <div className="mt-8 flex gap-4 justify-center">
            ...
            >
              {rematchClicked ? `Čekam ostale... (${readyCount}/4)` : "Revanš"}
            </button>
            <button
              onClick={handleLeave}
            ...
            >
              {leaveClicked ? "Izlazim..." : "Izađi iz igre"}
            </button>
          </div>
          {readyCount > 0 && readyCount < 4 && rematchClicked && (
            <div className="text-white/60 text-sm mt-2">Spremno: {readyCount}/4 igrača</div>
          )}
        </div>
      )}
    </div>
...
// (removed) TableDecor: props are now user-selected via "Uzmi piće"
      ...
      <div className="relative w-full max-w-3xl rounded-2xl bg-neutral-950 ring-1 ring-white/10 p-4">
        ...
        <div className="mt-4 max-h-[70vh] overflow-auto">
          {list.length ? (
            ...
            <div className="text-sm text-white/70">Još uvek niste nosili karte.</div>
          )}
        </div>
      </div>
    </div>
      ...
      <div className="w-full max-w-xl rounded-2xl bg-white/5 ring-1 ring-white/10 p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-white/70 text-sm mt-1">Online kartaška igra 2v2 (osnovni engine)</div>
          </div>
        ...
        >
          {joining ? "Povezivanje..." : "Uđi / napravi sobu"}
        </button>

        <div className="mt-6">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold">Igrači u sobi</div>
            <div className="text-xs text-white/60">Igra kreće kada se povežu 4 igrača.</div>
          </div>
            ...
            {Array.from({ length: 4 }).map((_, i) => {
              ...
              return (
                <div key={i} className="rounded-xl bg-black/25 ring-1 ring-white/10 p-3">
                  ...
                  {p ? <div className="text-xs text-white/60">{teamLabel(p.team, players)}</div> : <div className="text-xs text-white/40">Čeka se...</div>}
                </div>
              );
            })}
          </div>
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-xl rounded-2xl bg-white/5 ring-1 ring-white/10 p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <SeatBadge label={missing === 0 ? "Start" : `Čeka se još: ${missing}`} active={missing === 0} />
        </div>

        <div className="mt-5 text-sm text-white/70">
          Igra počinje automatski kada se povežu 4 igrača. Timovi su fiksni: Tim A (1 i 3), Tim B (2 i 4).
        </div>
...
function Game({ state, playerId, socket }) {
  ...
  useEffect(() => {
    ...
    // FX
    if (deal.isLast) {
      setFx({ id: deal.id, kind: "deal", title: "POSLEDNJE DELJENJE", subtitle: "Nema više karata u špilu", durationMs: 2000 });
  ...
  useEffect(() => {
    ...
    if (a.zinga === 10) {
      ...
      setFx({ id: a.id, kind: "zinga", title: "ZINGA NA ŽANDARA! +20", subtitle: a.playerName });
    }
  ...
  function playCard(card) {
    ...
    socket.emit("game:play", { cardId: card.id }, (res) => {
      if (!res?.ok) setActionError(res?.error || "Greška.");
    });
  }
  ...
  return (
    <div className="min-h-screen p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-2xl bg-neutral-950 ring-1 ring-white/10 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-lg font-semibold">Uzmi piće</div>
                <div className="text-xs text-white/60">Svi vide šta si uzeo.</div>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {[
                { id: "spricer", label: "Špricer", img: imgSpricer },
      ...
      <CapturedCardsModal
        ...
        title={`Nošene karte (${myTeamLabel})`}
        cards={myCaptured}
      />
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="text-2xl font-semibold">Zinga</div>
            <SeatBadge label={`Vaš tim: ${myTeamLabel}`} active />
            ...
            <div className="relative z-10 p-4 h-[72vh] min-h-[520px]">
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-[min(820px,92%)]">
                  ...
                  <div className="flex items-end gap-3">
                    ...
                    >
                      ...
                      <div className="text-left">
                        <div className="font-semibold leading-tight">Naše karte</div>
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  ...
                  {myHand.length === 0 ? <div className="text-white/70 text-sm">Nemate karata (čekanje deljenja)...</div> : null}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-white/5 ring-1 ring-white/10 p-4 h-fit">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div className={["rounded-2xl p-3 ring-1", myTeam === "A" ? "bg-emerald-400/10 ring-emerald-400/20" : "bg-black/20 ring-white/10"].join(" ")}>
                ...
                <div className="text-xs text-white/60">
                  Karte: {g?.captures?.A?.cardsCount ?? 0} • Zinga: {g?.captures?.A?.zinga10 ?? 0} • Žandar: {g?.captures?.A?.zinga20 ?? 0}
                </div>
              </div>
              <div className={["rounded-2xl p-3 ring-1", myTeam === "B" ? "bg-emerald-400/10 ring-emerald-400/20" : "bg-black/20 ring-white/10"].join(" ")}>
                ...
                <div className="text-xs text-white/60">
                  Karte: {g?.captures?.B?.cardsCount ?? 0} • Zinga: {g?.captures?.B?.zinga10 ?? 0} • Žandar: {g?.captures?.B?.zinga20 ?? 0}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-white/70">
              <div className="rounded-2xl bg-black/20 ring-1 ring-white/10 p-3">
                <div className="text-white/60">Špil</div>
...
export default function App() {
  ...
  function ensureSocket() {
    ...
    s.on("connect_error", () => setError("Ne mogu da se povežem sa serverom."));
